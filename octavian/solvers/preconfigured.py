"""Preconfigured impulsive transfer solvers built on ASSET's UpdatedInterface.
...
This module owns Octavian's built-in, preconfigured two-impulse problem shapes.
It is separate from the composable compiler: callers provide a typed problem
specification, and this backend builds one of the fixed ASSET OCP layouts below.
Everything remains dimensional at the user API and relies on ASSET auto-scaling.
"""
...
class RendezvousResult:
    """Result returned by ASSET-backed transfer solvers.
...
def solve(
    ...
    *,
    ...
    """Solve a preconfigured transfer specification with the matching backend.

    This dispatcher is intentionally small: the typed spec determines which
    fixed OCP layout is built. More open-ended mission scripts should enter
    through the composable backend instead.
...
def solve_two_impulse_free_time(
    ...
    *,
    ...
    """Solve a single-phase two-impulse transfer with bounded final time.

    The initial guess is Lambert-seeded, then propagated into an ASSET LGL mesh
    with the Lambert departure velocity. Boundary position constraints are hard;
    optional velocity constraints are applied only when the spec disables the
    corresponding impulsive boundary. Delta-v objectives are expressed as
    boundary velocity mismatches.
...
def solve_two_impulse_precoast(
    ...
    *,
    ...
    """Solve a two-phase transfer with a variable coast before the impulse arc.

    The backend sweeps candidate precoast durations, propagates the initial
    state to each candidate link point, scores Lambert transfers from that point
    to the terminal position, and uses the best candidate to seed two ASSET
    phases. The compiled OCP then optimizes the precoast duration, transfer
    duration, link impulse, and optional boundary impulses together.
